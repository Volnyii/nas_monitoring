...
### Текущее состояние (обновляется каждые 60 сек)
...
cron (каждую минуту, от root)
  → /usr/local/bin/nas-dashboard.sh
      → vmstat, free, uptime, sensors, nvidia-smi   — системные метрики
      → lsblk                                        — список всех дисков
      → smartctl (пробует 7 флагов для каждого диска) — SMART данные
      → дописывает строку в /var/www/nas-dashboard/metrics.csv
